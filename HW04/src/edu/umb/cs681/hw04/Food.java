...
public class Food {
    ...
    public static void main(String args[]) {
        ...
        Food highestCaloriesItem = breakfast.stream().max(Comparator.comparing(Food::getCalories)).get();
        System.out.println("---Food item that has highest calories intake: "+ highestCaloriesItem.getName());

        List<Food> lowFatItems = breakfast.stream().filter((Food food) -> food.getFat() < 10).collect(Collectors.toList());
        System.out.println("---Low fat food items: ");
        lowFatItems.forEach((Food food) -> System.out.println(food.getName() + " " + food.getFat()));

        List<Food> sortedByProtein = breakfast.stream().sorted(Comparator.comparing(Food::getProtein, Comparator.reverseOrder())).collect(Collectors.toList());
        ...
        double totalCalories = breakfast.stream().map(Food::getCalories).reduce(0, (result, calories) -> result+=calories);
        System.out.println("---Total calories intake: " + totalCalories);

        double totalCarb = breakfast.stream().map(Food::getCarb).reduce(0.0, (result, carb) -> result+=carb, (finalResult, intermediateResult)->finalResult);
        System.out.println("---Total carb intake: " + totalCarb);

        double totalProtein = breakfast.stream().map((Food food) -> food.getProtein()).reduce((result, protein) -> result+=protein).get();
        System.out.println("---Total protein intake: " + totalProtein);

        double totalFat = breakfast.stream().map((Food food) -> food.getFat()).reduce((result, fat) -> result+=fat).get();
